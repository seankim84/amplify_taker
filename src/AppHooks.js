...
const AppHooks = () => {
   ...
   useEffect(() => {
    getNotes();
    const createNoteListener = API.graphql(graphqlOperation(onCreateNote)
    ).subscribe({
      next: noteData => {
...
const handleChangeNote = event => this.setNote( event.target.value )
        ...
        <div>
          {notes.map(item => (
            <div key={item.id} className="flex items-center">
              ...
              <button onClick={() => handleDeleteNote(item.id)} className="bg-transparent
              bn f4">
                <span>&times;</span>
              </button>
            </div>
